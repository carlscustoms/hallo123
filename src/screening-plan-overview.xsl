...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    <xsl:template match="//movies/movie">
        <xsl:call-template name="movies"></xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="main | support">
        <xsl:variable name="actortag" select="name(.)"></xsl:variable>
        <xsl:variable name="aid" select="@actor_ref"></xsl:variable>

        <xsl:if test="$actortag = 'main'">
            <b>Main:</b>
        </xsl:if>
        <xsl:if test="$actortag = 'support'">
            <b>Support:</b>
        </xsl:if>
        ...
        <xsl:for-each select="//actors/actor">
            <xsl:if test="$aid = @aid">
                ...
                (Age: <xsl:value-of
                    select="year-from-date(current-date()) - year-from-date(@birth_date)"></xsl:value-of>)
                (Country: <xsl:value-of select="@birth_country"></xsl:value-of>)
                Number of acting roles:
                <xsl:value-of select="count(//movies/movie/roles/*[@actor_ref = $aid])"></xsl:value-of>
                <br></br>
            </xsl:if>
        </xsl:for-each>
        ...
        <xsl:for-each select="//cinema/movie">
            ...
            <xsl:if test="@movie_ref = $mid">
                ...
                <xsl:if test="(following-sibling::*[1]/name()='info')">
                    <b>Announced on: </b>
                    <xsl:value-of select="following-sibling::*/@announcement"></xsl:value-of>
                    <table>
                        <tbody>
                            <tr>
                                <th>
                                    Premiere Showing on
                                    <xsl:value-of select="following-sibling::*/premiere-date"></xsl:value-of>
                                    in Room
                                    <xsl:value-of select="following-sibling::*/premiere-room"></xsl:value-of>
                                </th>
                            </tr>
                            <tr>
                                <td>
                                    <b>Regular Showings:</b>
                                    Shown on
                                    <xsl:value-of select="following-sibling::*/screening-date"></xsl:value-of>
                            ...
                            <tr>
                                <td>
                                    <b>Shown in:</b>
                                    <xsl:value-of select="count(following-sibling::*[1]/room) + 1"></xsl:value-of> room(s)
                                </td>
                            </tr>
                        </tbody>
                    </table>
